...
| **A fake render passes as SA3.** Verified 2026-09-06: three runs with escalating SA3 environment — including one pointed at a service confirmed to report the `sa3-1.0.0` build — all produced **byte-identical** audio from a **FakeAdapter** the app discovered in another worktree, returning `{"cache":"miss","status":"ready"}` every time and saying nothing about being fake. The owner would have rated a model that never ran | **Every generative candidate must carry the serving service's `build` string AND evidence of job activity in that service's own log.** A `status: ready` result is not evidence that a model ran. Condition G is blocked until a render is proven to reach SA3 — see [evidence/rehearsal-2026-09-06/SA3-PREFLIGHT.md](evidence/rehearsal-2026-09-06/SA3-PREFLIGHT.md) |
