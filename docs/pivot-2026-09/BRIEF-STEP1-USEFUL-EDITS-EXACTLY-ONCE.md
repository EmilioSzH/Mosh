...
| 5 Deterministic balance | `native/matchers.ts` : `matchExplicitBalanceUtteranceV1`; `native/explicitBalance.ts`; `native/payloads.ts` positive examples; `ui/src/agent/skills.ts` : `trackVolumeReachedV1` tolerance (added 2026-09-05 by the audit's own repair recommendation); their tests; `ui/src/ui/AgentComposer.skillFoundry.test.ts` | `adjust_level` (relative: `lower/turn … down/raise … N dB` → `set_track_volume {db: before + delta}` clamped −60…+6, postcondition on the snapshot value); `send_level` (`set the <track> <bus> send to N dB`, `more/less <bus> on <track> [by N dB]`, default ±3 dB spoken in `say`; no send to that bus → `blocked(missing_target)`, never auto-creates); `repeat_last` ("another N dB", "same again") bound to the last completed adjust within the same project epoch and 10 min, else `blocked` |
| 6 Provenance | `src/moshops/MoshOps.h/.cpp` : `executeFromUi`, `execute`, `cmdBatchBegin/End`, `logLine`, the snapshot session block (`revision`); `src/moshops/AgentTxn.h` volatile list; `ui/src/agent/skillFoundry/contracts.ts` (provenance type); `src/app/SelfTest.cpp` STEP1-PROV section; `ui/src/store.ts` `exec`; `ui/src/agent/executor.ts`; `taskExec.ts`; `AgentComposer.tsx` skill env; `ui/src/agent/skillFoundry/atomicPlan.ts` `batch_begin` args and its callers (`native/explicitBalance.ts`, `native/loadNamedPlugin.ts`, `declarativeExecutor.ts`, `skillHarness.ts`, the skill runtime) for the provenance object; `ui/scripts/produceLiveRun.mts` + new `ui/src/agent/loop/produceBatchArgs.ts` (+ test); `ui/scripts/lib/companionClient.mts` (origin sibling); new tests `ui/src/store.execOrigin.test.ts`, `ui/src/agent/executor.origin.test.ts` | additive `origin` on every JSONL line (`"ui"` when the call came through `executeFromUi` with no envelope origin; else the envelope's origin: `agent_loop`, `studio_skill`, `macro`, `produce_driver`, `native`); `turn_id` on every line while `inBatch`; `run_id` + `prompt_version` on the driver's `batch_begin` only. Historical lines lack the fields ⇒ readers treat absence as `unknown`. No new collector or schema |
...
(5) The produce driver's `batch_begin` still says `source: "agent_loop"` while its envelopes
carry origin `produce_driver`; unify at the driver. (6) Loop skip events reach the transcript
and the progress stream but neither the drawer's task store nor the model's task context
renders them (one branch in `taskStore.ts`, one line in `renderTaskContext`). (7) The reverb
macro's reset point is keyed on the project file, so Save As re-captures it at the current
level (accepted). (8) Under add/insert/put a plug-in is claimed only by its exact name, or by
a prefix when "plugin" is said; "add Serum" for "Serum 2" now goes to the loop, which can
still load it. (9) `session.revision` had to join the transaction fingerprint's volatile list
(`AgentTxn.h`) — a rollback restores content and still advances it. (10) A balance ask whose
NAMED target is not a track hands the turn back (`unsupported`) so the router/loop can take it;
only a nameless ask with no selection keeps the select-a-track guidance.
